    ...
    if ($myAccessRights == 'command' && $hour <= 23 && $minute <= 55) {
        ...
        while (($all_eventDays = $data_server_eventDays->fetch_assoc()) != false) {
            ...
            if ($data_server_dayTimes->num_rows == 0) {
                ...
                $last_dayTimes_id = $mysqli->insert_id;
                $mysqli -> query("DELETE FROM `menuDishes` WHERE `dayTimesId` = '".$last_dayTimes_id."'");
                for ($i=0; $i < count($dishes); $i++) { 
                    $mysqli -> query("INSERT INTO `menuDishes` (`dayTimesId`, `dishId`) VALUES ('".$last_dayTimes_id."', '".$dishes[$i]."')");
                }
            }
